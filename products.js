...
function renderPackages(selector, packages = []) {
  document.querySelectorAll(selector).forEach(el => {
    el.innerHTML = Array.isArray(packages)
      ? packages.map(pkg => `<li class="cursor-pointer transition-colors list-none" data-price="${pkg.price}" data-size="${pkg.size}">${pkg.size} - ₹${pkg.price}</li>`).join('')
      : '';
  });
}
...
function initializeSwiper(product) {
  ...
  const hasMultipleSlides = Array.isArray(product?.media?.gallery) && 
...
function buildSwiperSlides(product) {
  ...
  const slides = [];
  const imgClass = 'z-10 w-[140px] md:w-[370px] 2xl:w-[400px] 5xl-lg:w-[600px] m-auto';
...
function populateNutrition(nutrition) {
  ...
  fields.forEach(field => {
    ...
    const value = data?.value ?? (field === 'servingSize' ? nutrition[field] : '');
    const percent = data?.percent ?? (field.includes('vitamin') || 
                    field === 'calcium' || field === 'iron' || 
                    field === 'potassium' ? '00 %' : '-');

    updateElements(`.${field.replace(/([A-Z])/g, '-$1').toLowerCase()}`, value);
    
    if (data?.percent !== undefined) {
      updateElements(`.${field.replace(/([A-Z])/g, '-$1').toLowerCase()}-percent`, percent);
    }
  });
  ...
  document.querySelectorAll('.nutrition-table').forEach(el => {
    el.innerHTML = `
      <table>
        ...
        <tr><th>Saturated Fat</th><td>${nutrition.saturatedFat?.value ?? ''}</td></tr>
        <tr><th>Trans Fat</th><td>${nutrition.transFat?.value ?? ''}</td></tr>
        <tr><th>Cholesterol</th><td>${nutrition.cholesterol?.value ?? ''}</td></tr>
        <tr><th>Sodium</th><td>${nutrition.sodium?.value ?? ''}</td></tr>
        <tr><th>Carbohydrates</th><td>${nutrition.carbohydrates?.value ?? ''}</td></tr>
        <tr><th>Dietary Fiber</th><td>${nutrition.dietaryFiber?.value ?? ''}</td></tr>
        <tr><th>Total Sugars</th><td>${nutrition.totalSugars?.value ?? ''}</td></tr>
...
function setupQuantityControls() {
  ...
  increaseBtns.forEach(btn => btn.addEventListener('click', () => updateQuantity(1)));
  decreaseBtns.forEach(btn => btn.addEventListener('click', () => updateQuantity(-1)));
}
...
async function loadProduct() {
  try {
    ...
    const product = products.find(p => 
      String(p.id) === String(id) && p.slug === name
    );
    ...
    const firstPackageSize = product?.buy?.packages?.[0]?.size || product?.buy?.defaultSize;
